    ...
    static orgCreate = asyncHandler(
        async (req: RequestWithDecodedOrg, res: Response, next: NextFunction) => {
            ...
            if (errors.length > 0) {
                ...
                return next(new ErrorResponse("finish date must bigger than start date", 404));
            } else if (tour.startDate <= new Date()) {
                return next(new ErrorResponse("you can't create tour with before today date", 404));
            } else {
                await tour.save();
            }
            res.json({
